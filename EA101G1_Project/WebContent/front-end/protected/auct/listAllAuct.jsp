		...
		<table class="table table-striped table-dark text-center">
			...
			<tbody>
				...
				<c:forEach var="auctVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
					<tr>
						<th  style="vertical-align: middle;">${auctVO.auct_id}</th>
						...
						<td>
							...
							%>
							<c:if test="${auctVO.auct_down == 1 && auctVO.auct_sold == 0}">
								<!-- 下架 且 未售出 -->
							...
							<c:if test="${auctVO.auct_down == 0 && now_ms < startTime2}">
								<!-- 上架 且 開始前 -->
								<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/auct/auct.do" style="margin-bottom: 0px;">
									<input class="btn btn-info" type="submit" value="修改">
									<input type="hidden" name="auct_id" value="${auctVO.auct_id}">
							...
							<c:if test="${auctVO.auct_down == 0 && now_ms >= startTime2}">
								<!-- 上架 且 開始後 -->
								<input class="btn btn-dark" type="submit" value="修改" disable onclick="showAlert();">
							</c:if>

							<c:if test="${auctVO.auct_down == 1 && auctVO.auct_sold == 1}">
								<!-- 下架 && 售出 -->
								<input class="btn btn-dark" type="submit" value="修改" disable onclick="showAlert2();">
							</c:if>
						...
						<td>
							...
							<c:if test="${auctVO.auct_down == 0 && now_ms >= startTime2}">
								<!-- 上架 且 拍賣開始 -->
								<input class="btn btn-dark" type="submit" value="下架" disable onclick="showAlert3();">
							</c:if>

							<c:if test="${auctVO.auct_down == 1 }">
								<!-- 下架  -->
								<input class="btn btn-dark" type="submit" value="下架" disable onclick="showAlert4();">
							</c:if>
